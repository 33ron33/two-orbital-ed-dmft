...
class SparseExactDiagonalization(object):
    ...
    def _calculate_density_matrix(self):
        ...
        U_h = self.U.H if hasattr(self.U, 'H') else self.U.T.conjugate()
        self.rho = np.einsum('ij,j,jk->ik', self.U, exp_bE, U_h)
    ...
    def get_expectation_value_sparse(self, operator):
        ...
        for idx in range(self.E.size):
            vec = np.asmatrix(self.U[:, idx]).reshape((-1, 1))
    ...
    def get_frequency_greens_function_component(self, iwn, op1, op2, xi):
        ...
        dE = (-self.E[:, None] + self.E[None, :]).ravel()  # (N^2,)
        exp_bE = np.exp(-self.beta * self.E)
        M = exp_bE[:, None] - xi * exp_bE[None, :]        # (N, N)

        op1_eig, op2_eig = self._operators_to_eigenbasis([op1, op2])

        # Pre-contract frequency-independent weights; original einsum was
        # 'nm,mn,nm,znm->z' so op2_eig enters transposed.
        W = np.asarray(op1_eig) * np.asarray(op2_eig).T * M  # (N, N)
        del op1_eig, op2_eig, M

        # Flatten to 1-D so each frequency step is a sequence of dot products.
        # Matsubara freqs are purely imaginary → denominator is never zero.
        w_flat = W.ravel()                   # (N²,) complex

        # Chunk the denominator allocation. For Nb=2, a full complex N²
        # temporary is ~256 MB per frequency; chunks keep the peak bounded.
        chunk_size = 1_048_576
        G = np.empty(len(iwn), dtype=complex)
        for iz, z in enumerate(iwn):
            acc = 0.0j
            for start in range(0, dE.size, chunk_size):
                stop = min(start + chunk_size, dE.size)
                acc += np.dot(w_flat[start:stop], 1.0 / (z - dE[start:stop]))
            G[iz] = acc / self.Z
